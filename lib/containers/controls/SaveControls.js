  ...
  */
  ...
  this.addToStorage = function (key, value) {
    ...
    if (!items) {
      ...
      var parsedItems = _this2.retrieveFromStorage(key);
  ...
  this.popFromStorage = function (key) {
    ...
    var items = _this2.retrieveFromStorage(key);
    var poppedValue = null;

    if (items && !index) {
      // Key exists in storage
      poppedValue = items.pop();
      sessionStorage.setItem(key, _this2.stringify(items));
    } else if (items && index) {
  ...
  this.addVisited = function (href) {
    _this2.addToStorage('visited', href);
  };

  this.getNewPage = function () {
    return _this2.popFromStorage('newPages');
  };
  ...
  this.getAllClickableElements = function () {
    ...
    for (var i = 0; i < allElements.length; i++) {
      ...
      if (element.onclick && !_this2.checkClicked(element)) {
  ...
  this.checkNewPage = function (newHref) {
    ...
    if (currentHref !== newHref) {
      var visited = _this2.retrieveFromStorage('visited');
      if (visited) {
        var index = visited.indexOf(currentElement.href);
        if (index !== -1) {
          _this2.popFromStorage('visited', index);
        }
      }
...
>>>>>>> buidling
  ...
  this.hasVisited = function (state) {
    var href = state.href;
    var visited = _this2.retrieveFromStorage('visited');
    var pages = _this2.retrieveFromStorage('newPages');
  ...
  this.handleLoad = function () {
    ...
    if (resume === 'true') {
      ...
      _this2.addToStorage('visited', window.location.href);
      _this2.clickAllElements();
    }
  };

  this.saveError = function (api, data, error) {
    _this2.addToStorage('errors', { api: api, data: data, error: error });
  };
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 21:
              ...
              _this2.saveError(api, data, _context2.t0.toString());
