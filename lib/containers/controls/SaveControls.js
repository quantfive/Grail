...
var SaveControls = (_temp = _class = function (_Component) {
  ...
  function SaveControls(props) {
    ...
    if (resume === 'true') {
      ...
      window.XMLHttpRequest.prototype.send = _this.send;
  ...
  */
  ...
  this.clickAllElements = function () {
    ...
    window.XMLHttpRequest.prototype.send = _this3.send;
    window.XMLHttpRequest.prototype.oldSend = oldSend;
...
>>>>>>> added a way to ignore elements and building
  ...
  this.send = function (data) {
    var isGrail = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

    window.XMLHttpRequest.prototype.oldSend = oldSend;
    var scThis = _this3;
    var grailActions = scThis.props.grailActions;

    var api = 'xml';
    var type = null; // How to get request type?
    ...
    if (isGrail) {
      ...
      try {
        ...
        api = _this3.responseUrl;
        _this3.saveError(api, data, error.toString());
      }
      var event = {
        endpoint: api,
