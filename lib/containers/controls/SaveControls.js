...
var SaveControls = (_temp = _class = function (_Component) {
  ...
  */
  ...
  this.addVisited = function (href) {
    ...
    if (!visited) {
      var visitedJson = JSON.stringify([href]);
      sessionStorage.setItem('visited', visitedJson);
    } else {
      var visitedPages = JSON.parse(visited);
      visitedPages.push(href);
      var _visitedJson = JSON.stringify(visitedPages);
      sessionStorage.setItem('visited', _visitedJson);
    }
  };
  ...
  this.getNewPage = function () {
    ...
    if (!newPages) {
      ...
      var pages = JSON.parse(newPages);
      var newPage = pages.pop();
      var elementJson = JSON.stringify(pages);
    ...
    if (currentHref !== newHref) {
      ...
      if (visited) {
        ...
        if (index !== -1) {
          ...
          var visitedJson = JSON.stringify(visitedPages);
          sessionStorage.setItem('visited', visitedJson);
        }
      }
      ...
      if (!newPages && hasVisited) {
        var elementJson = JSON.stringify([currentElement.href]);
        sessionStorage.setItem('newPages', elementJson);
      } else if (hasVisited) {
        var pages = JSON.parse(newPages);
        pages.push(currentElement.href);
        var _elementJson = JSON.stringify(pages);
        sessionStorage.setItem('newPages', _elementJson);
      }
