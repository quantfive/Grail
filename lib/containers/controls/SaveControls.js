...
var SaveControls = function (_Component) {
  ...
  function SaveControls() {
    ...
    _this.afterClick = function (state, currentHref, fetchDone, elements) {
      if (!_this.state.fetchMade || fetchDone) {
        var grailActions = _this.props.grailActions;

        var newHref = window.location.href;
        // grailActions.addClicked(state);
        _this.addVisited(newHref, state);
        // this.addChildrenStates(state);
        _this.checkNewPage(currentHref, newHref, state);
        _this.clickAll3(elements);
      }
    };

    _this.addVisited = function (href, state) {
    ...
    _this.getNextState = function () {
      var grailActions = _this.props.grailActions;

      var clickedStates = grailActions.getClicked().clickedStates;
      var state = null;
      ...
      if (state === null || state === undefined) {
        state = _this.getNewPageStates();
      }

      return state;
    };

    _this.clickAll2 = function () {
      var grailActions = _this.props.grailActions;

      window.fetch = _this.fetch;
      var state = _this.getNextState();
      ...
      if (state && state.onclick) {
        ...
        try {
          state.click();
          var timeout = setTimeout(_this.afterClick.bind(_this, state, currentHref), 10);
        } catch (e) {
          alert(e);
    ...
    _this.clickAll3 = function (elements) {
      ...
      if (element !== null && element !== undefined) {
        ...
        try {
          element.click();
          _this.afterClick2(element, currentHref, elements);
        } catch (e) {
          alert(e);
        }
      } else {
    ...
    _this.afterClick2 = function (state, currentHref, elements) {
      if (!_this.state.fetchMade || fetchDone) {
        var grailActions = _this.props.grailActions;

        var newHref = window.location.href;
        _this.addVisited(newHref, state);
        _this.checkNewPage2(currentHref, newHref, state);

        // Need this timeout so window.history.back can load;
        var timeout = setTimeout(_this.clickAll3.bind(_this, elements), 100);
      }
    };

    _this.checkNewPage2 = function (currentHref, newHref, state) {
    ...
    _this.handleLoad = function () {
      ...
      if (resume === 'true') {
        ...
        // debugger;
        // this.clickAll();
        _this.clickAll3(_this.getAllClickableElements());
      }
    };
    ...
    _this.fetch = function () {
      var _ref2 = _asyncToGenerator( /*#__PURE__*/_regenerator2.default.mark(function _callee2(api, data) {
        ...
        var grailActions, response, clonse, res, event;
